...
export function getMockTshirtData() {
  const tshirts = [
    {
    ...
    },
    {
      ...
      analytics: {
        ...
        monthlyTrend: [
          ...
          { month: 'Dec', sales: 1920, revenue: 1920 },
...
function computeHealthScore(tshirt: any): number {
  const stock = tshirt.analytics.totalStock;
  const sales = tshirt.analytics.monthlyTrend.reduce((sum: number, m: any) => sum + m.sales, 0);
  if (stock === 0) return 0;
  const ratio = sales / stock;
  return Math.min(100, Math.round(ratio * 50) + 50);
}

function computeStockAging(tshirt: any): number {
